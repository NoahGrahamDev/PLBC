...
export default function Services() {
  const services = [
    {
      icon: (
        <svg className="w-12 h-12 text-[#ce6712]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section id="services" className="bg-white py-16 lg:py-24">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div key={index} className="bg-gray-50 rounded-xl p-8 text-center hover:shadow-lg transition-shadow duration-300">
              ...
              <p className="text-lg font-semibold text-[#ce6712] mb-4">
                {service.time}
              </p>
        ...
        <div className="text-center mt-12">
          <a 
            href="#contact" 
            className="inline-flex items-center bg-[#ce6712] hover:bg-[#b85a0f] text-white px-8 py-4 rounded-lg font-semibold text-lg transition-colors shadow-lg"
          >
            Plan Your Visit
